...
export class ProductServiceStub {
  ...
  getBySubCatPaged(productSubCategoryId: number, page: number, itemsPerPage: number) {
    ...
    var r: PagedResults = {
        ...
        currentPage: page,
        pageCount: (l % itemsPerPage > 0 ? Math.floor(l/itemsPerPage) + 1 : Math.floor(l / itemsPerPage)),
        rowCount: l,
        lastRowOnPage: (page * itemsPerPage > l ? l : page * itemsPerPage),
        firstRowOnPage: (page * itemsPerPage) - (itemsPerPage - 1),
...
export class OpenEditModalMock {
  open() {
    return {
      afterClosed: () => of({})
    };
